 ...
 * 作者姓名 修改时间 版本号 描述
 */
package ETL.com.xdjy.map_side.answer_exam_question_diff_mark_cate;
...
public class MyMapJoin {
    // 定义输入路径
    private static String INPUT_PATH1 = "hdfs://192.168.198.60:9000/xdjy/result/answer_exam_question_diff_mark/answer_exam_question_diff_mark.log";
    //加载到内存的表的路径
    private static String INPUT_PATH2 = "hdfs://192.168.198.60:9000/xdjy/category/category.log";
    // 定义输出路径
    private static String OUT_PATH = "hdfs://192.168.198.60:9000/xdjy/result/answer_exam_question_diff_mark_cate";
...
// 把字符数组中有用的数据存在一个Map中
                    String key_part = splits[0];
                    String m_res =splits[1];
                    joinData.put(key_part,m_res);
//                    System.out.println(key_part+"  "+m_res);
                }
            } catch (Exception e) {
                e.printStackTrace();
            } finally{
...
//
// 获取关联字段depNo，这个字段是关键
            String temp_type  = values[12];
// 根据depNo从内存中的关联表中获取要关联的属性depName
            String res = joinData.get(temp_type);
